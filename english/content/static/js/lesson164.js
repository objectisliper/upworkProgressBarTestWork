...
var Lesson164 = {
	...
	chBg: function()
	{
		...
		if(fin_exam164[nq].user_result == undefined)
		{
			...
			Course.oState.card164 = Lesson164.nCorrAnsw / fin_exam164.length * 100;
			console.log("%%:",Course.oState.card164, Lesson164.nCorrAnsw, fin_exam164.length);
			OCookies.set("card164", Course.oState.card164);
		}
	},
	...
	pre: function()
	{
		...
		if(Course.oState.card164 < 1)
		{
			Course.oState.card164 = 1;
			OCookies.set("card164", Course.oState.card164);
		}
	...
	next: function(n)
	{
		switch(n)
		{
			case 0:
				...
				$('#lesson164 .title h3 span.q').html(1);
				$('#lesson164 .title h3 span.n').html(fin_exam164.length);

				Course.showLesson(164, function()
				{
					//attempts
					Course.oState.card164n++;
					OCookies.set("card164n", Course.oState.card164n);
					console.log("164 attempt",Course.oState.card164n);

					if(Course.oState.card164n > 5)
					{
						//too many attempts!
						Course.oState.card164n = 0; //reset attempts
						OCookies.set("card164n", Course.oState.card164n);

						//message: You did not manage to complete the challenge in a reasonable number of attempts.
						Course.greyPopup(164, 2, true);
						return;
					}
					//
			...
			default: //1..9 - go to the next question
				...
				$('#lesson164 .d-exam').fadeOut(500, function()
				{
					...
					$('#lesson164 .title h3 span.q').html(n+1);
	...
	next: function(n)
	{
		switch(n)
		{
			case 0:
				...
				Course.oState.card164 = Lesson164.nCorrAnsw / fin_exam164.length * 100;
console.log("end %%:",Course.oState.card164, Lesson164.nCorrAnsw, fin_exam164.length);
				OCookies.set("card164", Course.oState.card164);
